...
ReflexInstallNavViewCore = function(deps)
    ...
    local function NavPartialPinColor(col)
        ...
        if maxc == 0 or delta == 0 then
            local v = math.floor(maxc * 0.765 + 0.5)
            return (v << 24) | (v << 16) | (v << 8) | alpha
        end
        ...
        local sat = delta / maxc
        local val = maxc / 255
        sat = math.min(1, sat + (1 - sat) * 0.225)
        val = math.max(0, math.min(1, val * 0.765))
    ...
    local COL_TREE_ARROW_REST = rgb(0x25272C)
          ...
          if nav_render_expanded then
              ...
              for mi, md in ipairs(mini_items) do
                  ...
                  elseif item.tree_partial then
                      r.ImGui_DrawList_AddCircleFilled(dl, dot_x + dot_r, dot_cy, pin_overlay_r, NavPartialPinColor(circ_col), nav_circle_segments)
                  end
                  r.ImGui_PopID(ctx)
        ...
        if current_page == "tracks" then
          ...
          for ri, item in ipairs(render_list) do
              ...
              elseif item.kind == "folder" and item.tree_partial then
                  r.ImGui_DrawList_AddCircleFilled(dl, circ_cx, mid_y, pin_dot_r, NavPartialPinColor(circ_col), nav_circle_segments)
              end
